...
const ChatMessages = ({ messages }) => {
    ...
    return (
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-6 w-full">
            <div className="max-w-xl md:max-w-4xl mx-auto space-y-6">
                {(!currentChat || messages.length === 0) ? (
                    ...
                    messages.map((msg) => (
                        ...
                        >
                            {/* Message Content */}
                            <div
                                className={`min-w-0 w-full ${msg.role === 'user'
                                    ? 'bg-[#2d2f31] text-gray-100 rounded-3xl rounded-tr-sm px-4 py-2 sm:px-5 sm:py-2 shadow-xl max-w-[95%] sm:max-w-[85%] md:max-w-[70%]'
