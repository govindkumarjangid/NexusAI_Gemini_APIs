...
const LoginPage = () => {
  ...
  const labelClass = "block text-xs font-semibold mb-1.5 ml-2 tracking-wide text-(--text-secondary)";
  const inputClass = "w-full px-4 py-3 rounded-full text-sm outline-none transition-all duration-200 border focus:ring-3 bg-(--bg-elevated) border-(--border-color) text-(--text-primary) focus:border-(--accent-color) ring-[color-mix(in_srgb,var(--accent-color)_30%,transparent)]";
  const inputAllMotionAnimation = { initial: { opacity: 0, x: -16 }, animate: { opacity: 1, x: 0 }, transition: { delay: 0.35 } };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden px-4 py-8 bg-(--bg-base)">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          className="absolute w-75 h-75 rounded-full -top-[8%] -right-[4%] animate-[orb-float_10s_ease-in-out_infinite] bg-[radial-gradient(circle,color-mix(in_srgb,var(--accent-color)_30%,transparent),transparent_70%)] blur-[70px]"
        />
        <div
