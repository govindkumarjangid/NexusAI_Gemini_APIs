...
const RegisterPage = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden px-4 py-8 bg-(--bg-base)">
      ...
      {/* Card */}
      <motion.div
        className="w-full sm:max-w-md sm:rounded-2xl px-2 sm:px-10 py-7 sm:py-10 relative z-10 border-0 sm:border backdrop-blur-none sm:backdrop-blur-xl bg-transparent sm:bg-[color-mix(in_srgb,var(--bg-surface)_80%,transparent)] border-(--border-color)"
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
            <button
              ...
              className="w-full py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all duration-300 hover:-translate-y-px active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer bg-(--accent-color) text-(--accent-text-color)"
            >
